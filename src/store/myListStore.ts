...
interface MyListState {
  ...
  addToList: (anime: Anime, status: WatchStatus, userScore?: number | null) => void;
...
export const useMyListStore = create<MyListState>()(
  persist(
    (set, get) => ({
      list: {},
      addToList: (anime, status, userScore) =>
        set((state) => ({
          list: {
            ...state.list,
            [anime.mal_id]: createListEntry(anime, status, userScore),
          },
        })),
